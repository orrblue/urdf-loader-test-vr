...
export default class Task {
    ...
    this.fsm = new StateMachine({
      ...
      transitions: [
        ...
        },
        {
          ...
          from: "*",
          to: function () {
            return Number(this.state) === that.numRounds - 1
              ? "COMPLETE"
              : `${Number(this.state) + 1}`;
          },
        },
        ...
        onNext: (state) => {
          ...
          const data = [
            [
              ...
              that.name,
              `${Number(state.from) + 1}`,
              JSON.stringify({ timeElapsed: t - this.startTime }),
            ],
          ];
  ...
  log(t) {
    this.data.log(
      this.currentTrialId,
      this.getState((t - this.startTime) / 1000)
    );
  }
  ...
  getState(t) {
    ...
    const state = [
      time,
      {
        ...
        rotation: {
          x: headset.quaternion.x,
          y: headset.quaternion.y,
          z: headset.quaternion.z,
          w: headset.quaternion.w,
        },
      ...
      },
      {
        ...
        rotation: {
          x: controllerLeft.quaternion.x,
          y: controllerLeft.quaternion.y,
          z: controllerLeft.quaternion.z,
          w: controllerLeft.quaternion.w,
        },
        scale: {
      ...
      },
      {
        ...
        rotation: {
          x: controllerRight.quaternion.x,
          y: controllerRight.quaternion.y,
          z: controllerRight.quaternion.z,
          w: controllerRight.quaternion.w,
        },
        scale: {
