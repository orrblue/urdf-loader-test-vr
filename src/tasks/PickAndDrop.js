...
export default class PickAndDrop extends Task {
  ...
  constructor(params, condition, options) {
    super("pick-and-drop", params, condition, options, [
      () => {
        ...
        this.objects.blocks[2].set({ position: new T.Vector3(0.7, 3, 0) });
        this.objects.box.set({ position: new T.Vector3(0.7, 3, -0.5) });
      },
      () => {
        this.objects.blocks[0].set({ position: new T.Vector3(1, 3, 0.0) });
        this.objects.blocks[1].set({ position: new T.Vector3(0.8, 3, 0.3) });
        this.objects.blocks[2].set({ position: new T.Vector3(0.7, 3, -0.2) });
        this.objects.box.set({ position: new T.Vector3(0.7, 3, 0.5) });
      },
    ]);
  }
  ...
  async onStart() {
    ...
    this.instructions.appendChild(
      this.ui.createText("Pick and Drop\n", { fontSize: 0.08 })
    );
    this.instructions.appendChild(
      this.ui.createText(
        this.text ??
          "Complete the task by picking up the blocks with the robot and placing them in the box. Close the box after you are done.\n\n"
      )
    );
