...
export default class GraspingTutorial extends Task {
  static async init(params, condition, options = {}) {
    const task = new GraspingTutorial(params, condition, {
      numRounds: 2,
      resetAfterTrial: false,
    });
    task.objects = {};
    return task;
  }
  ...
  onStart() {
    this.openInstructions = this.ui.createContainer(
    ...
    this.openInstructions.appendChild(
      this.ui.createText("To open the gripper, squeeze the trigger again.", {
        fontSize: 0.025,
      })
    );

    this.closeInstructions = this.ui.createContainer(
    ...
    this.closeInstructions.appendChild(
      this.ui.createText("To close the gripper, squeeze the trigger", {
        fontSize: 0.025,
      })
    );
  }
  ...
  onUpdate(t, info) {
    this.openInstructions
      .getObject()
      .position.copy(
        info.currEEAbsThree.posi.clone().add(new T.Vector3(0, 0.2, 0))
      );
    ...
    const grasping = this.condition.graspingControl;
