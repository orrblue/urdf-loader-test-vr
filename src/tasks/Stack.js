...
export default class Stack extends Task {
  ...
  async onStart() {
    ...
    this.instructions.appendChild(
      this.ui.createText("Block Stacking\n", { fontSize: 0.08 })
    );
    this.instructions.appendChild(
      this.ui.createText(
        this.text ??
          "Complete the task by stacking the blocks in any order.\n\n",
        { fontSize: 0.04 }
      )
    );
  ...
  onUpdate(t, info) {
    ...
    for (let i = 0; i < blocks.length - 1; i++) {
      ...
      if (
        blocks[i].meshes[0].position.y - blocks[i + 1].meshes[0].position.y >
        0.01
      ) {
        stackCount++;
      }
    ...
    if (stackCount > 1 && !this.fsm.is("COMPLETE")) {
      // TODO: fix this
      this.stackCounter.show();
      let position;
      for (const block of blocks) {
        if (!block.grasped) {
          position = block.meshes[0].position
            .clone()
            .add(new T.Vector3(0, 0.05, 0));
          break;
        }
      }
