...
export default class Controllers {
  constructor(renderer, teleportvr) {
    ...
    this.controller[0].grip.add(
      controllerModelFactory.createControllerModel(this.controller[0].grip)
    );
    this.controller[1].grip.add(
      controllerModelFactory.createControllerModel(this.controller[1].grip)
    );
  ...
  getDirection(hand) {
    const i = this.controller[0].hand === (hand ?? this.hand) ? 0 : 1;
    return this.controller[i].controller
  ...
  update() {
    ...
    if (
      this.get().gamepad.buttons[4].pressed &&
      this.get().gamepad.buttons[5].pressed
    ) {
