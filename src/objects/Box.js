...
import download from "../utilities/misc";
...
export default class Box extends SceneObject {
  ...
  async fetch() {
    const gltfs = [await loadGLTF(PATH_BOX), await loadGLTF(PATH_LID)];

    const meshes = [gltfs[0].scene, gltfs[1].scene];
