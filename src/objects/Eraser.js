...
const PATH = "./models/eraser.glb";

export default class Eraser extends SceneObject {
  constructor(params, options = {}) {
    super("eraser", params);
    this.initPosition = options.position ?? new T.Vector3();
    this.initRotation = options.rotation ?? new T.Euler();
    this.initScale = options.scale ?? new T.Vector3(0.0015, 0.0015, 0.0015);
    this.loaded = false;
  }

  static async init(params) {
    const object = new Eraser(params);
    await object.fetch();
    return object;
  }
