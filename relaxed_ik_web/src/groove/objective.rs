...
pub fn swamp_loss(x_val: f64, t: f64, c: f64, d: f64, f: f64) -> f64 {
    // - f * (-2.0 * ((x_val - t) / c).powi(d) * ((x_val - t) / c).powi(2).exp() ).exp()
    f * ((x_val - t) / c).powf(d) 
}
...
impl ObjectiveTrait for MatchEEQuatGoals {
    fn call(&self, x: &[f64], v: &vars::RelaxedIKVars, frames: &Vec<(Vec<nalgebra::Vector3<f64>>, Vec<nalgebra::UnitQuaternion<f64>>)>) -> f64 {
        // d: vector in the world coordinate frame
        ...
        disp = angle_between_quaternion(goal_quat, ee_quat);
        disp2 = angle_between_quaternion(goal_quat, ee_quat2);
