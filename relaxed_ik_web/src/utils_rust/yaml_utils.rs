...
pub struct CuboidConstructorData {
    pub name: String,
...
#[derive(Serialize, Deserialize)]
pub struct SpheresConstructorData {
    pub name: String,
    pub scale: f64,
    pub translation: Vector3<f64>,
    pub animation: String
...
#[derive(Serialize, Deserialize)]
pub struct EnvCollisionFileParserConstructorData {
    pub cuboids: Vec<CuboidConstructorData>,
    pub spheres: Vec<SpheresConstructorData>,
    pub link_radius: f64
}
...
impl EnvCollisionFileParser {
    pub fn new(config: EnvCollisionFileParserConstructorData) -> Self {
        let mut cuboids: Vec<CuboidEnv> = Vec::new();
        let mut spheres: Vec<SphereEnv> = Vec::new();

        let cuboids_list = config.cuboids;
        let spheres_list = config.spheres;
        // let spheres_option = config.as_vec();

        for i in 0..cuboids_list.len() {
            let name = cuboids_list[i].name.clone();
            // let name = format!("Object_{}", i.to_string());
            let scale = cuboids_list[i].scale;
            let sx_half = scale.x;
            let sy_half = scale.y;
            let sz_half = scale.z;

            let rots = cuboids_list[i].rotation;
            let rx = rots.x;
            let ry = rots.y;
            let rz = rots.z;

            let ts = cuboids_list[i].translation;
            let tx = ts.x;
            let ty = ts.y;
            let tz = ts.z;
        ...
        for i in 0..spheres_list.len() {
            let name = spheres_list[i].name.clone();
            let radius = spheres_list[i].scale;

            let ts = spheres_list[i].translation;
            let tx = ts.x;
            let ty = ts.y;
            let tz = ts.z;
        ...
        Self{robot_link_radius: config.link_radius, 
            cuboids, spheres}
    }
    // pub fn from_yaml_path(fp: String) -> Self {
    //     let fp2 = fp.clone();
